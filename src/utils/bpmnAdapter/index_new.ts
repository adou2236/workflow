...
const defaultRetainedProperties = ['properties', 'startPoint', 'endPoint', 'wayPoints'];
...
function convertLf2ProcessData(bpmnData: any, data: any, other?: ExtraPropsType) {
  ...
  console.log(bpmnData, data, other);
...
function convertLf2DiagramData(bpmnDiagramData: any, data: any) {
  bpmnDiagramData['bpmndi:BPMNEdge'] = data.edges.map((edge: any) => {
    ...
    const wayPoints = edge.wayPoints.map(({ x, y }: { x: number; y: number }) => ({
      '-x': x,
      '-y': y,
    }));
...
function getEdgeConfig(edgeValue: any, processValue: any) {
  ...
  properties = convertXmlToNormal(properties);
  const wayPoints = edgeValue['di:waypoint'].map((point: any) => ({
    x: Number(point['-x']),
    y: Number(point['-y']),
  }));
