...
enum BpmnElements {
  ...
  END = 'bpmn:endEvent',
  EXCLUSIVEGATWAY = 'bpmn:exclusiveGateway',
...
function convertLf2DiagramData(bpmnDiagramData, data) {
  bpmnDiagramData['bpmndi:BPMNEdge'] = data.edges.map((edge) => {
    ...
    if (edge.text?.value) {
      // label位置固定，转换通过计算得出
      const textDom = document.querySelector(`[jtk-overlay-id="${edge.id}__label"]`)!;
      const textWidth = textDom?.clientWidth || 0;
      const textHeight = textDom?.clientHeight || 10;
      diagramData['bpmndi:BPMNLabel'] = {
        'dc:Bounds': {
          '-x': textDom.offsetLeft,
          '-y': textDom.offsetTop,
          '-width': textWidth,
          '-height': textHeight,
        },
  ...
  bpmnDiagramData['bpmndi:BPMNShape'] = data.nodes.map((node) => {
    ...
    let width = NODE_SIZE;
    let height = NODE_SIZE;
    const { x, y } = node.bound;
...
function getNodeConfig(shapeValue, type, processValue) {
  ...
  if (name) {
    ...
    if (shapeValue['bpmndi:BPMNLabel'] && shapeValue['bpmndi:BPMNLabel']['dc:Bounds']) {
      ...
      text.bound.x = Number(textBounds['-x']) + Number(textBounds['-width']) / 2;
      text.bound.y = Number(textBounds['-y']) + Number(textBounds['-height']) / 2;
    }
  }
...
function getEdgeConfig(edgeValue, processValue) {
  ...
  const edge: EdgeConfig = {
    ...
    wayPoints: edgeValue['di:waypoint'].map((point) => ({
      x: Number(point['-x']),
      y: Number(point['-y']),
    })),
...
class BpmnAdapter {
  ...
  adapterOut = (data, retainedFields?: string[]) => {
    ...
    convertLf2DiagramData.call(this, bpmnDiagramData, data);
...
BpmnAdapter.shapeConfigMap.set(BpmnElements.EXCLUSIVEGATWAY, {
  width: ExclusiveGatewayConfig.width,
  height: ExclusiveGatewayConfig.height,
});
BpmnAdapter.shapeConfigMap.set(BpmnElements.INCLUSIVEGATEWAY, {
  width: InclusiveGatewayConfig.width,
  height: InclusiveGatewayConfig.height,
});
BpmnAdapter.shapeConfigMap.set(BpmnElements.PARALLELGATEWAY, {
